...
fun isComplete(secret: String, currentGuess: String) = secret==currentGuess.replace(" ","")
...
fun generateNewUserWord(secret: String, guess: Char, currentUserWord: String): String{
    ...
    if (guess in secret){
        for(i in 0 until secret.length) {
            if (secret[i]==guess){
                newUserWord=newUserWord.plus("${secret[i]}$separator")
            } else {
                newUserWord=newUserWord.plus("${currentUserWord[i * 2]}$separator")
            }
        }
        newUserWord=newUserWord.removeSuffix("$separator")
        return newUserWord
    }
...
fun getHiddenSecret(wordLength: Int): String{
    ...
    for (i in 0 until wordLength){
        a=a.plus("_$separator")
    }
    a=a.removeSuffix(separator)
    return a
}
fun isCorrectInput(userInput: String): Boolean{
    ...
    if (userInput.filter{it->!it.isLetter()}.isNotEmpty()){
...
fun getRoundResults(secret: String, guess: Char, currentUserWord: String): String{
    if (guess in secret.filter{it !in currentUserWord}){
...
fun playGame(secret: String, maxAttemptsCount: Int){
    ...
    } while (!isComplete(secret, currentUserWord))
}
fun main() {
